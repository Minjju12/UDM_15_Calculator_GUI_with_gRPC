...
class Calculator:
    ...
    def _build_display(self):
        ...
        tk.Label(frame, textvariable=self.expr_var,font=("Segoe UI", 11), bg=BG, fg="#777777",anchor="e").pack(fill="x")
        #số hiển thị số
    ...
    def _build_buttons(self):
        ...
        for (text, row, col, color, rs, cs) in layout:
            btn = tk.Button(frame, text=text,bg=color, fg=FG,font=("Segoe UI", 12, "bold"),relief="flat", bd=0,activebackground="#484848",cursor="hand2")
            btn.grid(row=row, column=col,rowspan=rs, columnspan=cs,padx=3, pady=3, sticky="nsew")
